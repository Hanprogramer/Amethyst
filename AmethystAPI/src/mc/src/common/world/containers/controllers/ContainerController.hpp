...
public:
    ...
    /// @vidx {1}
    MC virtual void* getRecipeItem(int); // probably returns ItemStack or ItemStackBase
    /// @vidx {2}
    MC virtual bool canRemove(int, int);
    /// @vidx {3}
    MC virtual bool isItemAllowed(const ItemStackBase&);
    /// @vidx {4}
    MC virtual bool isItemFiltered(const Recipes&, const ItemStackBase&);
    /// @vidx {5}
    MC virtual int getBackgroundStyle(int, bool);
    /// @vidx {6}

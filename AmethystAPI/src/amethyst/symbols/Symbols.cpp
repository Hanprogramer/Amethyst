...
constexpr auto BDSAPI_FAKEDLL_NAME = "AmethystRuntime.dll";
...
{
    Log::Info("__delayLoadHelper2 called for {}", PFromRva<LPCSTR>(pidd->rvaDLLName));
    ...
    if (!pImgDelayDescr_BDS && !strcmp(dli.szDll, BDSAPI_FAKEDLL_NAME)) {
        pImgDelayDescr_BDS = pidd;
    }

    if (pImgDelayDescr_BDS == pidd) {
        //pfnRet = (FARPROC)resolveSymbol(dli.dlp.szProcName);
        //Log::Info("Resolving Symbol: {}", dli.dlp.szProcName);
        Amethyst::SymbolLoader& symbolLoader = Amethyst::GetSymbolLoader();
        std::string symbol = dli.dlp.szProcName;
        pfnRet = (FARPROC)symbolLoader.ResolveSymbol(symbol);
...
ExternC PfnDliHook __pfnDliNotifyHook2 = nullptr; // NOLINT(misc-misplaced-const)

ExternC PfnDliHook __pfnDliFailureHook2 = nullptr; // NOLINT(misc-misplaced-const)
